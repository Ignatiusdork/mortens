...
export default function Home() {
    ...
    */
    const setENSOrAddress = async (address, webProvider) => {

        // Lookup the ENS related to the given address
        var _ens = await webProvider.lookupAddress(address);
    ...
    */
    ...
    const getProviderOrSigner = async () => {
        ...
        // Connect to MetaMask
        // SInce we store `web3Modal` as refrence, we need to acess the `current` value to grt acess to teh underlying object
        ...
        if (chainId != 5) {
            window.alert("Chanmge the network to Goerli");
            ...
            <div className={styles.main}>
                <div>
                    <h1 className={styles.title}>
                        Welcome to Cyborg's Punks {ens ? ens : address}!
                    </h1>
                    <div className={styles.description}>
                        {/* Using HTML Entities for the apostrophe */}
                        It&#39; an NFT collection for Cyborgs Punks
                    </div>
                    {renderButton()}
                </div>
            </div>
